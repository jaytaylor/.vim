...
function! PhpCleaner()
    ...
    %s/}\s*else\s*\(\(if\)\s*\((.*)\)\)\?\s*{/} else\1 {/ge
    "%s/}\s*else\s*\(if\)\?\s*{/} else\1 {/ge

    %s/\(if\|for\|foreach\|while\)\s*(\s*/\1 (/ge

    %s/try\s*{/try {/ge
    %s/}\s*catch\s*(/} catch (/ge

    %s/){/) {/ge

    %s/\(class\)\s\+\([a-zA-Z0-9_]\+\) *\( extends \+[a-zA-Z0-9_]\+\| implements \+[a-zA-Z0-9_]\+\)\?s*\({\)/\1 \2\3 \4/ge

    %s/\(function\)\s\+\([a-zA-Z0-9_]\+\)\s*\((.*)\)\s*\({\)/\1 \2\3 \4/ge

    %s/\(require\|include\)\(_once\)\?\s*(\s*\(.*\)\s*)\s*\(;\)/\1\2 \3\4/ge

    %s/<?= */<?php echo /ge 

    %s/ \+$//ge

    " Make default arguments not have a space in the value separator..
    %s/\(function\s*[^(]\+(\([^ ]\| [^=]\| =[^ ]\)*\)\( = \)/\1=/ge
    %s/\(function\s*[^(]\+(\([^ ]\| [^=]\| =[^ ]\)*\)\( = \)/\1=/ge
    %s/\(function\s*[^(]\+(\([^ ]\| [^=]\| =[^ ]\)*\)\( = \)/\1=/ge
    %s/\(function\s*[^(]\+(\([^ ]\| [^=]\| =[^ ]\)*\)\( = \)/\1=/ge
